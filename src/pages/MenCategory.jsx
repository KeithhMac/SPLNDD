...
export const MenCategory = () => {
      ...
      try {
        ...
        if (res.data?.success && Array.isArray(res.data.feedbacks)) {
          const mapped = res.data.feedbacks.map((f) => ({
            id: f.id,
            name: f.display_name,                 // backend already normalized ("Anonymous" when needed)
  ...
  useEffect(() => {
    const fetchCategoryProducts = async () => {
      try {
        const res = await api.get(`/customer/products/men/${type}`);
  ...
  if (loading || showSkeleton) {
    ...
    return (
      <main className="min-h-screen overflow-x-hidden flex flex-col gap-[2.1rem] mb-[5rem] ">
        <div
  ...
  return (
    <main className="min-h-screen overflow-x-hidden flex flex-col gap-[2rem] ">
      ...
      >
        <div className="flex flex-col justify-center items-center">
          ...
          <p className="max-w-[32rem] mt-4 mx-[2rem] text-center">
            Discover the essence of style with our signature men's hoodies —
            oversized, zipped or pullover, crafted for modern comfort.
          </p>
        </div>
      </div>
                ...
                >
                  ...
                  <div className="p-4 flex flex-col gap-2">
                    <h2 className="text-lg font-semibold text-black">
                      {variant.product_name}{variant.option_values && Object.keys(variant.option_values).length > 0 && (
    ...
    ${stockLeft <= 0 || spinningId === variant.variant_id
                          ...
                          : "bg-[#D5A58B] hover:bg-[#CFAF8B] shadow-md hover:shadow-lg cursor-pointer"
                        }
  `}
                    >
