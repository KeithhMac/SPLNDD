...
export const WomenCategory = () => {
      ...
      try {
        ...
        if (res.data?.success && Array.isArray(res.data.feedbacks)) {
          const mapped = res.data.feedbacks.map((f) => ({
            id: f.id,
            // Trust backend normalization: display_name is already "Anonymous" when needed
            name: f.display_name,
  ...
  // Fixed price slider bounds
  const PRICE_MIN = 0;
  const PRICE_MAX = 100000;

  // Filters (local-only)
  const [priceRange, setPriceRange] = useState([PRICE_MIN, PRICE_MAX]);
  // { Color: ["Pink","Black"], Size: ["M"], ... }
  const [selectedOptions, setSelectedOptions] = useState({});

  useEffect(() => {
    const fetchCategoryProducts = async () => {
      try {
        const res = await api.get(`/customer/products/women/${type}`);
        ...
        for (const p of fetched) {
          const cartItem = cart.find(
            (item) => item.variant_id === p.variant_id
          );
          const remainingStock =
            (p.display_stock ?? 0) - (cartItem?.quantity || 0);
  ...
  const availableOptions = useMemo(() => {
    ...
    for (const v of products) {
      // normal variant options (Size, Color, etc.)
      if (v?.option_values) {
        for (const [opt, val] of Object.entries(v.option_values)) {
          if (!map[opt]) map[opt] = new Set();
          if (val != null) map[opt].add(val);
        }
      }

      // ✅ add tags as a filter option
      if (Array.isArray(v.tags)) {
        if (!map["Tags"]) map["Tags"] = new Set();
        v.tags.forEach((tag) => {
          if (tag != null) map["Tags"].add(tag);
        });
      }
    }
  ...
  // Apply filters: price (fixed), then options (OR within, AND across)
  const filteredProducts = useMemo(() => {
    return products.filter((v) => {
      ...
      for (const [optName, chosen] of Object.entries(selectedOptions)) {
        ...
        if (optName === "Tags") {
          // ✅ filter by tags
          if (!tags.some((t) => chosen.includes(t))) return false;
        } else {
          // ✅ filter by Size, Color, etc.
          const val = variantOptions[optName];
  ...
  function OptionSelectMulti({
    ...
    // We manage selection manually; shadcn Select is used only for the dropdown UI
    const toggle = (val) => {
              ...
              {items.map((val) => {
                ...
                return (
                  <label
                    ...
                    onClick={(e) => {
                      e.preventDefault(); // stop default item behavior
                      toggle(val);
                    }}
                  >
                    <Checkbox
  ...
  return (
    <main className="min-h-screen overflow-x-hidden flex flex-col gap-[2rem] ">
      <div
        className="w-full h-[15rem] bg-cover bg-top  flex flex-wrap items-center justify-center text-white rounded-2xl my-[1rem] bg-black/20 bg-blend-overlay bg-fixed"
        style={{ backgroundImage: `url(${intro_bg})` }}
      >
        <div className="flex flex-col justify-center items-center">
          <h1 className="text-2xl font-bold capitalize">
            SPLND'D — {formattedType}
          </h1>
          <p className="max-w-[32rem] mt-4 mx-[2rem] moveFromBottom text-center">
            Get a closer look at our collection of oversized hoodies, both
            zipped and pullover. Browse our styles and pick your favorite.
          </p>
        </div>
      </div>
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="w-[20rem] p-[1rem]">
              ...
              <div className="px-4 py-4 overflow-y-auto max-h-[calc(100vh-112px)] space-y-6">
                ...
                {/* Select dropdowns (multi-check inside) */}
                  ...
                  <div className="flex flex-col gap-4">
                    {Object.entries(availableOptions).map(
                      ([optName, values]) => (
                        <OptionSelectMulti
                          key={optName}
                          label={optName} // e.g., "Color", "Size"
                          items={values} // e.g., ["Red","Green","Blue"]
              ...
              <div className="p-4 border-t mt-[5rem]">
                <Button
                  className="w-full bg-[#D5A58B] hover:bg-[#e0b9a4]"
          ...
          <>
            <div className="mt-6 flex flex-wrap justify-center gap-8">
              {paginatedProducts.map((variant) => {
                ...
                const images = Array.isArray(variant.image_urls)
                  ? variant.image_urls.slice(0, 2)
                  : [];
                const stockLeft =
                  localStockMap[variant.variant_id] ?? variant.display_stock;

                return (
                  <div
                    key={variant.variant_id}
                    className="w-[15rem] shadow-lg bg-white rounded-md overflow-hidden"
                  >
                    <NavLink
                      to={linkPath}
                    ...
                    <div className="p-4 flex flex-col gap-2">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          setSpinningId(variant.variant_id); // start spinner
                          addToCart(variant);
                          setLocalStockMap((prev) => ({
                            ...prev,
                            [variant.variant_id]: Math.max(
                              0,
                              (prev[variant.variant_id] ?? 0) - 1
                            ),
                          }));
  ...
  `}
          ...
          <p className="mt-6">No products found with current filters.</p>
        )}
      </section>
