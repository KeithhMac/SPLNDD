    ...
    const checkOrder = async () => {
      try {
        const res = await api.get(
          `${import.meta.env.VITE_SERVER_URL}/customer/orders/${orderRef}?status=success`,
  ...
  return (
    <main className="w-full h-auto p-4 flex justify-center items-center">
      <section className="mt-40 bg-[#fffcf2] shadow-lg p-6 flex flex-col gap-4 justify-center items-center w-full max-w-md rounded-lg border border-gray-200">
        <h2 className="text-xl font-semibold">We've got your order!</h2>
        ...
        <Button className="w-full bg-[#d6a58b] hover:bg-[#dbaf98] cursor-pointer" onClick={() => navigate("/Order-History")}>
          View Order Details
        </Button>
        <Button
          variant="secondary"
          className="w-full bg-white hover:bg-gray-50 cursor-pointer"
          onClick={() => navigate("/")}
        >
          Continue Shopping
        </Button>
